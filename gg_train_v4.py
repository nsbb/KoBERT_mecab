...
def main():
    ...
    for e in range(num_epochs+1):
        ...
        m_path = '/toy/LG_model/'
    #torch.save(model,m_path,'LG_model' + tt  + '.pt')
        if e%10==0:
            torch.save(model,m_path+'kobert_model'+str(e)+'.pt')
    #torch.save(model.state_dict(), m_path + 'model_state_dict' + tt + '.pt')
            torch.save(model.state_dict(), m_path + 'kobert_state'+str(e)+'.pt')
    torch.save(model,m_path+'kobert_model'+str(e)+'.pt')
    torch.save(model.state_dict(),m_path+'kobert_state'+str(e)+'.pt')
